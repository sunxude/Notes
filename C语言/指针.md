...
```
#### \*cp++和 \*++cp的区别
\*cp++是先使用当前地址，然后解引用最后再指向下一个地址。
\*++cp是先指向下一个地址，再解引用现在地址的值
...
int main() {
    // 函数指针数组定义
    void (*func_array[2])(void) = {func_a, func_b};
...
void sx_printf(char **str, int n);
const char *str[] = {"Hello", "World","C Programming","Language","sx_printf Function"};
